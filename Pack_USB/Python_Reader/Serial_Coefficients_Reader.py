...
def lecture(nomOut):
    ...
    valeursB3 = list()
    valeursB4 = list()
    valeurs = [valeursB1, valeursB2, valeursB3, valeursB4]
    try:
        for i in range (int(nbSamples.get())):
            while True:                                  # Synchro
                if serialPortOpened.read() == b'S':
                    if serialPortOpened.read() == b'D':
                        break
            
            octets = serialPortOpened.read(16)           # Lecture de 16 octets
